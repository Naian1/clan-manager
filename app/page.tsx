...
export default async function HomePage() {
  ...
  const donationRanking = [...players].sort((a, b) => (b.donations ?? 0) - (a.donations ?? 0) || (a.clan_rank ?? 999) - (b.clan_rank ?? 999));
  ...
  const metrics = [
    ['players', 'Membros', clan?.members == null ? '— / 50' : `${clan.members} / 50`, clan ? 'Clã sincronizado' : '—'],
    ['wars', 'Guerras vencidas', clan?.war_wins ?? '—', clan ? 'Histórico oficial' : '—'],
    ['streak', 'Sequência de vitórias', clan?.win_streak ?? '—', 'Guerras do clã'],
    ['stars', 'Estrelas monitoradas', '—', 'Guerras + CWL'],
    ['donations', 'Doações na temporada', players.length ? totalDonations : '—', players.length ? `${players.length} jogadores` : '—'],
    ['received', 'Recebidas na temporada', players.length ? totalReceived : '—', players.length ? 'Dados atualizados' : '—'],
    ['raid', 'Capital Raid', clan?.capital_points ?? '—', clan?.capital_league ?? 'Pontos da Capital'],
