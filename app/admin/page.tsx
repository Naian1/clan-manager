...
export const metadata: Metadata = {
  title: 'Administração | Clan Manager',
  robots: {
    index: false,
    follow: false,
    nocache: true,
...
export default function AdminPage() {
  ...
  return <ClashShell active="admin" title="Administração" description="Acesso restrito para líderes e co-líderes do clã.">
    <section className="admin-grid"><article className="admin-card"><span className="eyebrow">INTEGRAÇÕES</span><h3>Conexões do sistema</h3><div className="check-row"><span>Supabase</span><span className={`badge ${configured ? '' : 'muted'}`}>{configured ? 'Configurado' : 'Pendente'}</span></div><div className="check-row"><span>API Supercell</span><span className={`badge ${supercellConfigured ? '' : 'muted'}`}>{supercellConfigured ? 'Configurada' : 'Pendente'}</span></div><div className="check-row"><span>RLS do banco</span><span className="badge">Ativo</span></div></article><article className="admin-card"><span className="eyebrow">SINCRONIZAÇÃO</span><h3>Atualizar dados do clã</h3><p>A sincronização é server-side, idempotente e registrada em <code>sync_runs</code>. A chave da Supercell nunca vai para o navegador.</p><p>As coletas automáticas mantêm membros, guerras e eventos históricos atualizados sem expor credenciais.</p></article><article className="admin-card"><span className="eyebrow">AUTORIZAÇÃO</span><h3>Allowlist de administradores</h3><p>O acesso administrativo será validado por autenticação e <code>admin_allowlist</code>. A página não é publicada na navegação nem deve ser indexada por buscadores.</p><a className="secondary-button" href="/api/health">Ver health check</a></article></section>
  </ClashShell>;
}
