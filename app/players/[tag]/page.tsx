...
import { getPlayer } from '../../../lib/clash/client';
...
function UnitList({ title, items, limit = 12 }: { title: string; items?: ClashPlayerItem[]; limit?: number }) {
  const visible = (items ?? []).filter(item => item.village !== 'builderBase').slice(0, limit);
...
export default async function PlayerPage({ params }: { params: Promise<{ tag: string }> }) {
  ...
  const averageDestruction = monitoredAttacks ? [...regularAttacks, ...cwlAttacks].reduce((sum, attack) => sum + Number(attack.destruction_percentage ?? 0), 0) / monitoredAttacks : 0;
  ...
  return <ClashShell active="players" title={live?.name ?? stored?.name ?? 'Jogador'} description={live?.tag ?? stored?.tag ?? playerTag}>
    <section className="player-hero-card">
      ...
      <div className="player-league-card">{league?.iconUrls?.medium || stored?.league_icon_url ? <img src={league?.iconUrls?.medium ?? stored?.league_icon_url} alt=""/> : null}<small>LIGA ATUAL</small><strong>{league?.name ?? stored?.league_name ?? '—'}</strong><span>{live?.attackWins ?? stored?.attack_wins ?? 0} vitórias ofensivas · {live?.defenseWins ?? stored?.defense_wins ?? 0} defensivas</span></div>
    </section>

    <section className="profile-monitor-grid">
      <article className="profile-monitor-card accent"><span className="eyebrow">PELO CLÃ · MONITORADO</span><h3>{monitoredStars} estrelas</h3><p>Exato a partir dos ataques arquivados pelo Clan Manager.</p><div className="profile-monitor-stats"><span><small>GUERRAS</small><b>{warsMonitored}</b></span><span><small>ATAQUES</small><b>{monitoredAttacks}</b></span><span><small>TRIPLES</small><b>{triples}</b></span><span><small>DESTRUIÇÃO</small><b>{averageDestruction.toFixed(1)}%</b></span></div></article>
      <article className="profile-monitor-card"><span className="eyebrow">BASELINE DA CONTA</span><h3>{baseline ?? '—'} War Stars</h3><p>War Stars totais quando o monitoramento começou em {formatDate(monitoringStarted)}.</p><div className="profile-monitor-stats"><span><small>CONTA AGORA</small><b>{officialWarStars ?? '—'}</b></span><span><small>VARIAÇÃO</small><b>{accountDelta == null ? '—' : `${accountDelta >= 0 ? '+' : ''}${accountDelta}`}</b></span><span><small>REGULAR</small><b>{regularStars}</b></span><span><small>CWL</small><b>{cwlStars}</b></span></div></article>
