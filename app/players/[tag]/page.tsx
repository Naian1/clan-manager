...
export default async function PlayerPage({ params }: { params: Promise<{ tag: string }> }) {
  ...
  const clanCapitalContributions = Number((live as (typeof live & { clanCapitalContributions?: number }) | null)?.clanCapitalContributions ?? 0);
  ...
  return <ClashShell active="players" title={live?.name ?? stored?.name ?? 'Jogador'} description={live?.tag ?? stored?.tag ?? playerTag}>
    ...
    <section className="profile-monitor-grid">
      ...
      <article className="profile-monitor-card"><span className="eyebrow">CAPITAL DO CLÃ</span><h3>{live ? clanCapitalContributions.toLocaleString('pt-BR') : '—'} contribuídos</h3><p>Total de Capital Gold contribuído pela conta, retornado ao vivo pela Supercell.</p><div className="profile-monitor-stats"><span><small>RAID LOOT MONITORADO</small><b>{capitalRaids.length ? capitalGold.toLocaleString('pt-BR') : '—'}</b></span><span><small>WEEKENDS</small><b>{capitalRaids.length || '—'}</b></span><span><small>ATAQUES RAID</small><b>{capitalRaids.length ? capitalAttacks : '—'}</b></span></div></article>
    </section>
